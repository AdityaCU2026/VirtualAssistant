...
const SignUp = () => {
    ...
    const handelSignUp = async(e) =>{
      ...
      setLoading(true);
        try{
          ...
          setLoading(false);
          navigate("/customize"); // Redirect to customize page after successful signup
        }
        catch(err){
          console.log("Aditya");
  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{backgroundImage:`url(${bg})`}}>
         ...
         justify-center items-center gap-[20px] px-[20px]' onSubmit = {handelSignUp}>
            <h1 className='text-white text-[30px] font-semibold mb-[30px]'>Register to <span className='text-gray-900'>Virtual Assistant </span></h1>
            <input type="text" placeholder='Enter your name' className='w-full h-[60px]
            outline-none border-2 border-white bg-transparent text-white placeholder-gray-300 px-[20px] py-[10px] rounded-full text-[18px]' required onChange={(e)=>setName(e.target.value)} value = {name}/>
